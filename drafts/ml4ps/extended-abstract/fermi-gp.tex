...
% $f(x) \sim \operatorname{GP}\left(m(x), k\left(x, x^{\prime}\right)\right)$
% %
% $m(x) =\mathbb{E}[f(x)], k\left(x, x^{\prime}\right) =\operatorname{cov}\left[f(x), f\left(x^{\prime}\right)\right]$
%
%
% \begin{equation}
%   \left[f\left(x_{1}\right), \ldots, f\left(x_{n}\right)\right] \sim \mathcal{N}(m, K)
% \end{equation}
%
...
% %
% \begin{equation}
%   k_{\text {Matérn }}\left(x, x^{\prime}\right)=\frac{2^{1-\nu}}{\Gamma(\nu)}\left(\frac{\sqrt{2 \nu}\left|x-x^{\prime}\right|}{l}\right)^{\nu} K_{\nu}\left(\frac{\sqrt{2 \nu}\left|x-x^{\prime}\right|}{l}\right)
% \end{equation}
% %
% with $\nu = 3/2$, which generalizes the more common exponential quadratic kernel (which corresponds to $\nu\rightarrow\infty$) and was found to work better in the current context owing to being less smooth. 
...
% While the marginal likelihood, the posterior and the predictive distribution all have closed-form Gaussian expressions, the cost of evaluating them scales poorly with the number of data points, which is impractical for many dataset.
...
% Traditionally, GP hyperparameters are learned through the marginal log-likelihood and function predictions at test points can be computed using the predictive distribution, which is all analytically tractable for Gaussian likelihoods. 
...
% It can be shown~\cite{wingate_automated_2013} that the difference between the ELBO and the log-evidence is given by the KL divergence between the posterior and variational distribution. Taking stochastic gradient steps in the space of variational and model parameters then results in the variational distribution approximating the `true' posterior more and more closely~\cite{hoffman_stochastic_2013}.

% The approximate density is then fit by maximizing a lower bound on the marginal log-likelihood, the evidence lower bound, $\mathrm{ELBO} \equiv \mathbb{E}_{q_{\phi}(z)}\left[\log p_{\theta}(d, z)-\log q_{\phi}(z)\right]$. It can be shown~\cite{wingate_automated_2013} that the difference between the ELBO and the log-evidence is given by the KL divergence between the posterior and variational distribution, $\log p_{\theta}(d)-\mathrm{ELBO}=D_\mathrm{KL}\left({q}_{\phi}(z) \| {p}_{\theta}(z \mid d)\right)$. Taking stochastic gradient steps in $\{\theta$-$\phi\}$ space then results in the variational distribution approximating the `true' posterior more and more closely~\cite{hoffman_stochastic_2013}.
...
%The strategy of Ref.~\cite{hensman2014scalable}, implemented in GPyTorch~\cite{gardner2018gpytorch}, is used to marginalize out the values of the GP at the inducing points by transforming a variational distribution over the GP values at the inducing points $q(u)$ to a variational distribution at the pixel locations $x_{p}$, \emph{i.e.} $q(f(x_{p})) = \int  \mathrm{d}u\, p(f(x_{p}), u)\,q(u)$.
% $q(u)=\mathcal N(m, S)$ (or write $\mathcal N(u \mid m, S)$) where $\mathbf{m} \in \mathbb{R}^{m}$ and $\mathbf{S} \in \mathbb{R}^{m \times m}$ 
...
% We take the following approach to allow for richer posterior distributions and include correlation effects between the GP and template parameter posteriors. 
...
% Template regression, Gaussian processes, link function, approximate methods, variational inference and ELBO optimization, subsampling, hyperparameter choice, building the posterior distribution and including correlations.
...
%An alternative prescription where a Gaussian prior on the hyperparameters with standard deviation of 20\% the best-fit value does not qualitatively change the results.
